...
from pages import home, discover, analytics, details
...
with st.sidebar:
    ...
    st.divider(); st.markdown("<span class='eyebrow'>Preferences</span>", unsafe_allow_html=True)
    st.selectbox("Language", ["English", "Hindi", "Tamil", "Telugu", "Korean", "Japanese"])
    st.toggle("Motion effects", value=True)
    if not client().available: st.caption("● Demo mode — TMDB key needed")
...
if "selected_movie" in st.session_state:
    ...
    st.title("Your saved movies"); st.caption("Your personal watchlist, stored privately on this device.")
