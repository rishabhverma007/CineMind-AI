...
def render(genre_map):
    ...
    query = st.text_input("", placeholder="Search a movie, actor, director, or an imperfect title…", label_visibility="collapsed")
    if query:
        log_search(query)
    ...
    if not api.available:
        st.info("Offline MovieLens mode is active — recommendations use 100,836 community ratings across 9,742 films. Add `TMDB_API_KEY` for posters, trailers and live TMDB discovery.")
